...
type UserQuery struct {
	...
	withUploadedReplayFiles *ReplayFileQuery
...
// QueryUploadedReplayFiles chains the current query on the "uploaded_replay_files" edge.
func (_q *UserQuery) QueryUploadedReplayFiles() *ReplayFileQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUploadedReplayFiles: _q.withUploadedReplayFiles.Clone(),
...
// WithUploadedReplayFiles tells the query-builder to eager-load the nodes that are connected to
// the "uploaded_replay_files" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithUploadedReplayFiles(opts ...func(*ReplayFileQuery)) *UserQuery {
	query := (&ReplayFileClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withUploadedReplayFiles = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [1]bool{
			_q.withUploadedReplayFiles != nil,
		}
	)
	...
	if query := _q.withUploadedReplayFiles; query != nil {
		if err := _q.loadUploadedReplayFiles(ctx, query, nodes,
			func(n *User) { n.Edges.UploadedReplayFiles = []*ReplayFile{} },
			func(n *User, e *ReplayFile) { n.Edges.UploadedReplayFiles = append(n.Edges.UploadedReplayFiles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadUploadedReplayFiles(ctx context.Context, query *ReplayFileQuery, nodes []*User, init func(*User), assign func(*User, *ReplayFile)) error {
	...
	query.Where(predicate.ReplayFile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UploadedReplayFilesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_uploaded_replay_files
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_uploaded_replay_files" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_uploaded_replay_files" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
