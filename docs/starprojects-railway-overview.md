...
| `stareplays-next` | `SUCCESS` | Next.js 운영 대시보드 | `stareplays.up.railway.app` |
...
| 경로 | 설명 |
| --- | --- |
| `frontend/app-next/app` | Next.js App Router route 엔트리포인트입니다. |
| `frontend/app-next/app/api/team-analysis/raw` | MCP와 LLM 분석용 raw JSON route입니다. |
| `frontend/app-next/components/dashboard` | 메인 대시보드 UI입니다. 최근 게임, 현재 유저, 업로드 흐름을 다룹니다. |
| `frontend/app-next/components/vault` | 게임 목록과 상세 replay 분석 UI입니다. |
| `frontend/app-next/components/analyzer` | 선택 게임의 replay analyzer 결과 UI입니다. |
| `frontend/app-next/components/rankings` | 3x3 랭킹과 종족 조합 승률 UI입니다. |
| `frontend/app-next/components/team-analysis` | 전체/시즌 팀 조합 분석, 선수 역량 오각형, insight 카드 UI입니다. |
| `frontend/app-next/components/seasons` | 시즌별 전적, 플레이어 승패/승률 추이, 경기 목록 UI입니다. |
| `frontend/app-next/components/shared` | `Panel`, `MetricCard`, `PlayerBadge`, `RaceBadge` 같은 공용 UI primitive입니다. |
| `frontend/app-next/lib/loaders` | route별 API 호출과 fallback 조립을 담당합니다. |
| `frontend/app-next/lib/adapters` | API 응답을 페이지 model로 변환합니다. |
| `frontend/app-next/lib/api` | 공통 fetch/action helper입니다. |
| `frontend/app-next/types` | API와 page model 타입입니다. |

주요 페이지:

| Route | 역할 | 주요 데이터 소스 |
| --- | --- | --- |
| `/` | 현재 사용자 기준 요약, 최근 게임, 업로드 진입점 | ranking, player stats, games |
| `/vault` | 게임 목록/상세 replay 분석 탐색 | `/api/v1/games`, `/api/v1/games/:id/detail` |
| `/analyzer` | analyzer 결과, player deep-dive, manual refresh | `/api/v1/games/:id/analyzer` |
| `/rankings` | 3x3 랭킹과 종족 조합 승률 | `/api/v1/rankings/3v3`, `/api/v1/analyzer/race-matchups` |
| `/team-analysis` | 전체 팀 조합/선수 역량/인사이트 | season/game list 기반 page model |
| `/seasons` | 시즌 목록과 시즌별 누적 전적 | `/api/v1/seasons` |
| `/seasons/[season]` | 특정 시즌 상세 분석 | `/api/v1/seasons`, game list fallback |
| `/api/team-analysis/raw` | MCP/LLM용 raw JSON | team-analysis page model |
...
- API-first 구조입니다. fixture는 운영 데이터가 없거나 read path가 실패했을 때의 fallback입니다.
- 서버 route에서 데이터를 읽어 안정적인 page model을 만든 뒤 client component로 전달합니다.
- 기존 `frontend/web`은 동작 참고용 legacy UI입니다. 새 화면은 `frontend/app-next`에서 일관된 디자인 시스템으로 구현합니다.
- 선수명은 한국어 별칭을 우선 사용하고, 플레이어/종족은 공용 badge 컴포넌트로 반복 표기합니다.
- 분석 수치에는 하드코딩된 운영 값 대신 API/model 값을 사용합니다.
- `/api/team-analysis/raw`는 team-analysis page model과 LLM prompt context를 함께 노출하므로, 여기에 민감 데이터가 포함되지 않도록 관리합니다.
...
```bash
curl -s https://stareplays.up.railway.app/api/team-analysis/raw \
