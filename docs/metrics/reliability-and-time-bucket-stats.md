...
- 값이 없거나 모두 0인 지표를 정규화 기본값으로 보정해 표시하지 않는다.
- 모든 선수 값이 동일한 지표는 비교 레이더에서 제외하거나 0으로 표시하고, “차이를 판별할 수 없음”으로 해석한다.
- 새 지표는 최소 1개 이상의 회귀 테스트에서 “데이터 없음일 때 점수가 부풀려지지 않음”을 검증한 뒤 화면에 추가한다.
- API raw payload에 값이 없으면 프론트에서 추정값을 만들지 않는다. 추정이 필요하면 별도 지표명과 품질 플래그를 붙인다.
