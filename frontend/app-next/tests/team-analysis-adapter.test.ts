...
describe("team analysis adapter", () => {
  ...
  it("aggregates only complete 3x3 official matches into player, race, lineup, and rating analysis", () => {
    ...
    expect(model.chartData.teamPentagon?.title).toBe("팀별 평균 피지컬 오각형");
    expect(model.chartData.teamPentagon?.axes).toEqual(["APM", "EAPM", "명령효율", "분당 생산", "자원 소모량"]);
    ...
    expect(Math.max(...teamApmScores)).toBe(100);
    expect(teamApmScores[1]).toBeCloseTo((rawLoserApmAverage / rawWinnerApmAverage) * 100, 1);
