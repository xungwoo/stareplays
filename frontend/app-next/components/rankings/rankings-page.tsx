...
import type { RankingsPageModel, RankingRow, RaceCompositionRow } from "@/types/rankings";

const CARD_STYLE = CYAN_PANEL_STYLE;

type RankingsSortBy = "win_rate" | "avg_apm" | "avg_eapm";
type RaceSortBy = "games" | "team_a_win_rate";

type RankingsPageViewModel = RankingsPageModel & {
...
function WinRateBar({ pct, color }: { pct: number; color: string }) {
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-1.5 rounded-full" style={{ backgroundColor: INNER_PANEL_STYLE.backgroundColor }}>
...
function formatArrow(isActive: boolean, isDescending: boolean) {
  if (!isActive) return "↕";
  return isDescending ? "▼" : "▲";
}

function sortRankings(rows: RankingRow[], sortBy: RankingsSortBy, sortDesc: boolean) {
...
function sortRaceCompositions(rows: RaceCompositionRow[], sortBy: RaceSortBy, sortDesc: boolean) {
  const dir = sortDesc ? -1 : 1;
      ...
      <div className="rounded-xl overflow-hidden" style={CARD_STYLE}>
        ...
        >
          ...
          <button
            ...
            className="text-[10px] font-mono font-semibold uppercase tracking-widest text-slate-600"
          >
...
function RaceCompositionTable({
  ...
  const raceMeta = raceCompositionsError
    ? `ERROR_LOAD_ANALYZER: ${raceCompositionsError}`
    : `TEAM_SIZE: 3v3 | QUALIFIED_GAMES: 48 | ROWS: ${sortedRaceCompositions.length}`;
    ...
    <div>
      ...
      <div className="rounded-xl overflow-hidden" style={CARD_STYLE}>
        ...
        >
          ...
          <button
            type="button"
            onClick={() => onSortChange("team_a_win_rate")}
            className="text-[10px] font-mono font-semibold uppercase tracking-widest text-slate-600"
          >
        ...
        {sortedRaceCompositions.length === 0 ? (
          ...
          sortedRaceCompositions.map((row, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: INNER_PANEL_STYLE.backgroundColor }}>
                  <div
